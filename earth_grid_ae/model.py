...
class Encoder(nn.Module):
    def __init__(self, in_channels, latent_size, kwargs):
        ...
        self.patch_1 = nn.Conv2d(
            in_channels, latent_size, kernel_size=4, stride=4, padding=(3, 0), **kwargs
        )
        self.patch_2 = nn.Conv2d(
            latent_size, latent_size, kernel_size=4, stride=4, padding=(3, 0), **kwargs
        )
        ...
        self.patch_5 = nn.Conv2d(
            latent_size, latent_size, kernel_size=2, stride=2, padding=(0, 1), **kwargs
        )
        self.patch_6 = nn.Conv2d(
            latent_size, latent_size, kernel_size=2, stride=2, padding=(0, 0), **kwargs
        )
        self.patch_7 = nn.Conv2d(
    ...
    def forward(self, x):
        ...
        decoder_2_raw = decoder_2.transpose(1, 2).unflatten(2, (46, 90))

        patch_3 = self.patch_3(decoder_2_raw).flatten(2).transpose(1, 2)
        decoder_3 = self.decoder_3(patch_3, decoder_2)
        decoder_3_raw = decoder_3.transpose(1, 2).unflatten(2, (23, 45))

        patch_4 = self.patch_4(decoder_3_raw).flatten(2).transpose(1, 2)
        decoder_4 = self.decoder_4(patch_4, decoder_3)
        decoder_4_raw = decoder_4.transpose(1, 2).unflatten(2, (12, 23))

        patch_5 = self.patch_5(decoder_4_raw).flatten(2).transpose(1, 2)
        decoder_5 = self.decoder_5(patch_5, decoder_4)
        decoder_5_raw = decoder_5.transpose(1, 2).unflatten(2, (6, 12))

        patch_6 = self.patch_6(decoder_5_raw).flatten(2).transpose(1, 2)
        decoder_6 = self.decoder_6(patch_6, decoder_5)
        decoder_6_raw = decoder_6.transpose(1, 2).unflatten(2, (3, 6))

        patch_7 = self.patch_7(decoder_6_raw).flatten(2).transpose(1, 2)
        decoder_7 = self.decoder_7(patch_7, decoder_6).flatten(1)

        x = self.linear(decoder_7)
...
class Decoder(nn.Module):
    def __init__(self, out_channels, latent_size, kwargs):
        ...
        self.deconv_7 = nn.ConvTranspose2d(
            latent_size,
            latent_size,
            kernel_size=2,
            stride=2,
        ...
        self.deconv_6 = nn.ConvTranspose2d(
            latent_size,
            latent_size,
            kernel_size=2,
            stride=2,
    ...
    def forward(self, x):
        ...
        x = (
            self.act(self.norm(self.deconv_7(x).flatten(2).transpose(1, 2)))
            .transpose(1, 2)
            .unflatten(2, (3, 6))
        )
        x = (
            self.act(self.norm(self.deconv_6(x).flatten(2).transpose(1, 2)))
            .transpose(1, 2)
            .unflatten(2, (6, 12))
        )
        x = (
