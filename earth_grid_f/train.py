...
class Trainer:
    ...
    def get_dataset(self):
        ...
        self.train_data = Dataset(self.args.latent_name, "train")
        self.val_data = Dataset(self.args.latent_name, "val")
    ...
    def build_model(self):
        ...
        self.model = Forecaster(self.args.latent_size, self.kwargs).to(**self.kwargs)
    ...
    def select_criterion(self):
        self.rmse = RMSE("earth_grid", self.device)
        self.acc = ACC("earth_grid", self.device)
        self.mean = torch.load(
            "/path/to/dataset/earth_grid/ref/mean.pt",
            map_location=self.device,
            weights_only=True,
        )
        self.std = torch.load(
            "/path/to/dataset/earth_grid/ref/std.pt",
            map_location=self.device,
            weights_only=True,
        )
    ...
    def val(self, epoch):
        with torch.no_grad():
            ...
            for i, data in enumerate(dataloader):
                ...
                predict_raw = predict_raw * self.std + self.mean
                target_raw = target_raw * self.std + self.mean

                loss_now = 0
                for j in range(self.shape[0]):
                    loss_now = loss_now - self.acc(
                        predict_raw[:, j], target_raw[:, j], self.mean[j]
                    )
    ...
    def test(self):
            ...
            for i, data in enumerate(dataloader):
                ...
                for j in range(input_raw.shape[0]):
                    ...
                    for k in range(self.shape[0]):
                        ...
                        rmse_now = self.rmse(predict_raw[j][k], target_raw[j][k])
                        rmse[k][index] += rmse_now

                        acc_now = self.acc(
                            predict_raw[j][k], target_raw[j][k], self.mean[k]
                        )
                        acc[k][index] += acc_now
        ...
        if self.rank == 0:
            ...
            for i in range(self.shape[0]):
                for j in range(self.val_data.future_num):
                    for k in range(self.args.world_size):
                        ...
                        rmse[i][j] += rmse_list[k][i][j]
                        acc[i][j] += acc_list[k][i][j]
            ...
            for i in range(self.shape[0]):
                for j in range(self.val_data.future_num):
                    logging.info(
                        f"Test channel {i + 1}, {self.val_data.future_range[j] * 6}h: num {cnt[i][j]}, "
                        f"MAE {mae[i][j]:.4f}, RMSE {rmse[i][j]:.4f}, ACC {acc[i][j]:.4f}"
                    )
