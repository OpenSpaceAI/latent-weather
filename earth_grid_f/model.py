...
from earth_grid_ae.model import Autoencoder
...
class Forecaster(nn.Module):
    def __init__(self, latent_size, kwargs):
        ...
        self.autoencoder = Autoencoder(latent_size, kwargs).requires_grad_(False)

        self.transformer = ForecasterTransformer(latent_size, kwargs)

    def forward(self, input, query):
        predict_latent = self.transformer(input, query)
