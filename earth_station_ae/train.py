...
class Trainer:
    ...
    def build_model(self):
        ...
        self.model = Autoencoder(self.kwargs).to(**self.kwargs)
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            mse = torch.zeros(self.shape[0], dtype=torch.float, device=self.device)
            ...
            for i, data in enumerate(dataloader):
                ...
                for j in range(input_raw.shape[0]):
                    for k in range(self.shape[0]):
                        ...
                        mse_now = MSE(predict_raw[j][k], input_raw[j][k])
                        mse[k] += mse_now
        ...
        if self.rank == 0:
            ...
            for i in range(self.shape[0]):
                logging.info(
                    f"Test channel {i + 1}: num {cnt[i]}, MAE {mae[i]:.4f}, MSE {mse[i]:.4f}"
                )
